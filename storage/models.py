...
class UserDB(Base):
    ...
    # ML задачи
    ml_tasks: Mapped[list["MLTaskDB"]] = relationship(
        back_populates="user",
    )
...
class MLTaskDB(Base):
    """ML-задачи для асинхронной обработки"""
    ...
    # Статус задачи
    status: Mapped[str] = mapped_column(String(50), default="pending")  # pending, running, done, failed, failed_queue
    ...
    # Результат (заполняется после обработки)
    ...
    # Связи
    user: Mapped["UserDB"] = relationship(back_populates="ml_tasks")
    model: Mapped["MLModelDB"] = relationship(back_populates="ml_tasks")
