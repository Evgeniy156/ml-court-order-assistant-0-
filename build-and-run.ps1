...
docker run -d `
  ...
  -e DATABASE_URL=postgresql://${env:POSTGRES_USER:-postgres}:${env:POSTGRES_PASSWORD:-postgres}@database:5432/${env:POSTGRES_DB:-ml_court} `
  -e SECRET_KEY=${env:SECRET_KEY:-your-secret-key-change-in-production} `
  -e RABBITMQ_HOST=rabbitmq `
  -e RABBITMQ_PORT=5672 `
  -e RABBITMQ_USER=${env:RABBITMQ_USER:-guest} `
  -e RABBITMQ_PASSWORD=${env:RABBITMQ_PASSWORD:-guest} `
